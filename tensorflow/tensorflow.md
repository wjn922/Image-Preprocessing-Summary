...
**参数：** `image` `uint8`型的`tensor`。3-D数组 [height, width, channels]。<br>
&emsp; &emsp; &thinsp; `format` string型，默认为`''`，表示为采用图片通道数。可选参数有: "", "grayscale", "rgb"。<br>
...
tf.image.convert_image_dtype(image,
			...
			name=None)
```
**功能：** 将解码得到的图片`uint8`型的`tensor`转化为其他类型`DType`。<br>
**参数：** `image``uint8`型`tensor`。<br>
&emsp; &emsp; &thinsp; `dtype`。需要转化的类型，如`tf.float32`，则数值将被归一化在[0,1]之间。<br>
&emsp; &emsp; &thinsp; `saturate`bool型，默认为`False`。<br>
&emsp; &emsp; &thinsp; `name`操作名（可选）。
